...
con= mysql.connector.connect(user="root", password="a1", host="localhost")
cur= con.cursor()
...
root.iconbitmap('H:\\4th Sem Study Material\\Project EAD+DBMS\\Code\\wedicon.png')
...
def weddingplanner():
	...
	def search():
		cur.execute("select count(*) from clients where Client_ID=%d ;"%int(clientt.get()))
		count= cur.fetchall()
		if(count[0][0]==1):
			global flag
			flag=1
